...
// Helper function to get MongoDB configuration dynamically
function getMongoDBConfig() {
  const MONGODB_URI = process.env.MONGODB_URI || process.env.MONGODB_URL || "mongodb://192.168.1.110:27017/anat_security?directConnection=true";
...
class MongoDBClient {
  ...
  private async _connect(): Promise<boolean> {
    try {
      // Get MongoDB configuration dynamically
      const { MONGODB_URI, DB_NAME, COLLECTION_NAME } = getMongoDBConfig();
