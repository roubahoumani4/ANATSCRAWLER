...
try:
    ...
    from .spiderfoot import SpiderFootHelpers  # type: ignore[import]
except Exception:
    try:
        # Try absolute import (for script context)
        from spiderfoot import SpiderFootHelpers  # type: ignore[import]
    except Exception:
        class SpiderFootHelpers:
            @staticmethod
            def cachePath():
                raise ImportError("SpiderFootHelpers could not be imported.")
            @staticmethod
            def urlBaseUrl(url):
                raise ImportError("SpiderFootHelpers could not be imported.")

# Ensure cryptography.x509.oid.NameOID is always imported for cert parsing
try:
...
class SpiderFoot:
    ...
    @property
    def socksProxy(self) -> Optional[str]:
        """SOCKS proxy
    ...
    @scanId.setter
    def scanId(self, scanId: Optional[str]) -> None:
        """Set the scan ID this instance of SpiderFoot is being used in.

        Args:
            scanId (Optional[str]): scan instance ID
        """
        self._scanId = scanId

    @socksProxy.setter
    def socksProxy(self, socksProxy: Optional[str]) -> None:
        """SOCKS proxy
        ...
        Args:
            socksProxy (Optional[str]): SOCKS proxy
        """
    ...
    def cacheGet(self, label: str, timeoutHrs: int) -> Optional[str]:
    ...
    def urlFQDN(self, url: str) -> Optional[str]:
        ...
        parts = baseurl.split('/')
        if len(parts) > count:
            return parts[count].lower()
        return None

    def domainKeyword(self, domain: str, tldList: list) -> Optional[str]:
    ...
    def hostDomain(self, hostname: str, tldList: list) -> Optional[str]:
    ...
    def isPublicIpAddress(self, ip: str) -> bool:
        ...
        # netaddr.IPAddress properties are attributes, not methods
    ...
    def normalizeDNS(self, res) -> list:
        ...
        # Accepts both list and tuple (from gethostbyname_ex/gethostbyaddr)
        if isinstance(res, tuple):
            # gethostbyname_ex/gethostbyaddr returns (hostname, aliaslist, ipaddrlist)
    ...
    def parseCert(self, rawcert: str, fqdn: Optional[str] = None, expiringdays: int = 30) -> Optional[dict]:
        ...
        try:
            ...
            self.error(f"Error loading certificate: {e}")
            return None
        ...
        try:
            ...
            if notafter_bytes:
                notafter = datetime.strptime(notafter_bytes.decode('utf-8'), "%Y%m%d%H%M%SZ")
        ...
        try:
            import cryptography.x509
            SubjectAlternativeName = getattr(cryptography.x509, 'SubjectAlternativeName', None)
            if SubjectAlternativeName is not None:
                ext = cert.extensions.get_extension_for_class(SubjectAlternativeName)
                for x in ext.value:
                    val = getattr(x, 'value', None)
                    if isinstance(val, str):
                        ret['altnames'].append(val.lower().encode('raw_unicode_escape').decode("ascii", errors='replace'))
        except Exception as e:
            self.debug(f"Problem processing certificate: {e}")

        certhosts = list()
        try:
            if NameOID is not None:
                attrs = cert.subject.get_attributes_for_oid(NameOID.COMMON_NAME)
                if len(attrs) == 1:
                    name = getattr(attrs[0], 'value', None)
    ...
    def fetchUrl(
        ...
        if sizeLimit is not None or headOnly:
            if noLog:
    ...
    def cveInfo(self, cveId: str, sources: str = "circl,nist") -> "Tuple[str, str]":
        ...
        """
        # Do not reassign sources to a list, keep original string for type safety
    ...
    def googleIterate(self, searchString: str, opts: Optional[dict] = None) -> Optional[dict]:
        ...
        try:
            ...
            if not content:
                self.error("The key 'content' in the Google API response is missing or empty.")
                return None
    ...
    def bingIterate(self, searchString: str, opts: Optional[dict] = None) -> Optional[dict]:
        ...
        if useragent is None or not isinstance(useragent, str):
            useragent = "SpiderFoot"
        headers = {"Ocp-Apim-Subscription-Key": opts.get("api_key")} if opts.get("api_key") else None
        response = self.fetchUrl(
        ...
        try:
            content = response.get('content')
            if not content:
                self.error("The key 'content' in the bing API response is missing or empty.")
                return None
            response_json = json.loads(content)
        except ValueError:
