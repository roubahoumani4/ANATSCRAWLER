...
from core.spiderfoot import logger as spiderfoot_logger


def startSpiderFootScanner(loggingQueue, *args, **kwargs):
    spiderfoot_logger.logWorkerSetup(loggingQueue)
...
class SpiderFootScanner():
    ...
    def __init__(self, scanName: str, scanId: str, targetValue: str, targetType: str, moduleList: list, globalOpts: dict, start: bool = True) -> None:
        ...
        self.__setStatus("INITIALIZING", time.time() * 1000, 0.0)
    ...
    @property
    def scanId(self) -> str:
        return self.__scanId or ""

    @property
    def status(self) -> str:
        return self.__status or ""

    def __setStatus(self, status: str, started: float = 0.0, ended: float = 0.0) -> None:
        ...
        if self.__dbh is not None and self.__scanId is not None:
            # scanInstanceSet expects all str arguments
            self.__dbh.scanInstanceSet(str(self.__scanId), str(int(started)), str(int(ended)), status)
    ...
    def __startScan(self) -> None:
        ...
        try:
            self.__setStatus("STARTING", time.time() * 1000, 0.0)
            if self.__sf and self.__target:
            ...
            for modName in self.__moduleList:
                ...
                if self.__config and isinstance(self.__config, dict) and self.__config.get('_socks1type', '') != '':
                    try:
                ...
                if self.__config and isinstance(self.__config, dict) and self.__config.get('__outputfilter'):
                    try:
                ...
                try:
                    ...
                    if newTarget is not None and isinstance(newTarget, SpiderFootTarget):
            ...
            # Only assign event queues if SpiderFootPlugin supports it and attribute is not None and is assignable
            try:
                if hasattr(psMod, 'outgoingEventQueue'):
                    setattr(psMod, 'outgoingEventQueue', self.eventQueue)
            except Exception:
                pass
            try:
                if hasattr(psMod, 'incomingEventQueue'):
                    setattr(psMod, 'incomingEventQueue', queue.Queue())
            except Exception:
                pass

            # Create the "ROOT" event which un-triggered modules will link events to
            # Create a dummy SpiderFootEvent for rootEvent with a valid sourceEvent
            dummy_source = SpiderFootEvent("DUMMY", self.__targetValue or "", "", None)  # type: ignore
            rootEvent = SpiderFootEvent("ROOT", self.__targetValue or "", "", dummy_source)
            ...
            if self.__targetType == 'INTERNET_NAME' and self.__sf and self.__sf.isDomain(self.__targetValue or "", self.__config['_internettlds'] if self.__config and '_internettlds' in self.__config else []):
                firstEvent = SpiderFootEvent('DOMAIN_NAME', self.__targetValue or "", "SpiderFoot UI", rootEvent)
                psMod.notifyListeners(firstEvent)
            ...
            # initializing
            scanstatus = self.__dbh.scanInstanceGet(str(self.__scanId)) if self.__dbh and self.__scanId is not None else None
            ...
            if not failed:
                self.__setStatus("FINISHED", 0.0, time.time() * 1000)
    ...
    def runCorrelations(self) -> None:
        ...
        if self.__sf and self.__config and '__correlationrules__' in self.__config:
            ...
            if self.__dbh and self.__scanId is not None:
                corr = SpiderFootCorrelator(self.__dbh, ruleset, str(self.__scanId))
    ...
    def threadsFinished(self, log_status: bool = False) -> bool:
        ...
        for mod in self.__moduleInstances.values():
            if mod.errorState and getattr(mod, 'incomingEventQueue', None) is not None:
                if self.__sf:
