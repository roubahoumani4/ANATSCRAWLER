...
echo "Starting OS Audit Agent..."
echo "Machine ID: $MACHINE_ID"
echo "Server: $SERVER_URL"

# Run Lynis audit
echo "Running Lynis security audit..."
AUDIT_OUTPUT=$(lynis audit system --quiet --quiet 2>&1 || true)

# Get system information
OS_INFO=$(lsb_release -d 2>/dev/null || echo "Unknown OS")
HOSTNAME=$(hostname)
...
echo "Sending audit report to server..."
curl -X POST "$SERVER_URL/api/v1/os-audit/reports" \\
  -H "Content-Type: application/json" \\
  -d @"$REPORT_FILE" || echo "Failed to send report"

# Cleanup
rm -f "$REPORT_FILE"

# Send heartbeat
echo "Sending heartbeat to server..."
curl -X POST "$SERVER_URL/api/v1/os-audit/agent/heartbeat" \\
  -H "Content-Type: application/json" \\
  -d "{\"agentInstallationToken\": \"$AGENT_TOKEN\"}" || echo "Failed to send heartbeat"

echo "OS Audit Agent completed successfully"
AGENT_SCRIPT
